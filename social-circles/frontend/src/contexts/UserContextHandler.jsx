...
/* Context provider for user components and pages */
const UserContextProvider = ({ children }) => {
  ...
  useEffect(() => {
    // Fetch user data from backend and store
    const getUserData = async () => {
      ...
      try {
        ...
        if (response.ok) { // Fetch successful, store user data
      ...
      } catch (error) { // Connected to server, but server error
        setError("Server error. Please contact the administrator");
      } finally {
        setLoading(false);
      }
